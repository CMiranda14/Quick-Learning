export const matriz_video_carbohidratos =[

    {titulo:'¿CUÁNTOS CARBOHIDRATOS NECESITAS CONSUMIR?',	texto:'La cantidad recomendada varía dependiendo de factores como la edad, género, actividad física, estado de salud y estado nutricional. Sin embargo, en general cerca de la mitad (50 a 65%) de la energía que el cuerpo necesita cada día debería provenir de carbohidratos, principalmente de los complejos, dejando solo un pequeño porcentaje (menos del 10%) para energía que venga de azúcares añadidos a los alimentos.'},
    {titulo:'¿QUÉ SON LOS CARBOHIDRATOS?',	texto:'Los carbohidratos, llamados también hidratos de carbono, son unas biomoléculas formadas por tres elementos fundamentales: el carbono, el hidrógeno y el oxígeno.'},
    {titulo:'¿POR QUÉ SON IMPORTANTES?',	texto:'Su principal función en el organismo de los seres vivos es la de contribuir en el almacenamiento y en la obtención de energía de forma inmediata, sobre todo al cerebro y al sistema nervioso.'},
    {titulo:'¿DÓNDE LAS ENCUENTRO?',	texto:'Algunos alimentos que son ricos en carbohidratos simples son las frutas y verduras, la leche y los productos derivados de esta como el queso o el yogur, así como en los azúcares y productos refinado. En cuanto a los carbohidratos complejos, se incluyen alimentos como legumbres, verduras ricas en almidón y panes y otros productos que incluyan cereales integrales.'},
    {titulo:'¿SABÍAS ESTO?',	texto:'Existen cuatro tipos de carbohidratos, en función de su estructura química: los monosacáridos, los disacáridos, los oligosacáridos y los polisacáridos.'},
];



export const matriz_video_proteinas =[

    {titulo:'¿CUÁNTAS PROTEÍNAS NECESITAS CONSUMIR?',	texto:'Es recomendable consumir proteínas de origen animal y vegetal, junto a una dieta balanceada que contenga todos los grupos de alimentos:La necesitas depende de las actividades diarias de cada persona, su etapa de vida y su peso. Es por es que tus necesidades proteicas son individuales. En general, se recomienda 1g de proteína por kilo en adultos sanos, por ejemplo'},
    {titulo:'¿QUÉ SON LAS PROTEÍNAS?',	texto:'Las proteínas son macromoléculas presentes en las células que coordinan múltiples funciones vitales. También se conocen como polipéptidos.'},
    {titulo:'¿POR QUÉ SON IMPORTANTES?',	texto:'Quizá no sepas mucho de ellas pero las proteínas son responsables de que podemos realizar nuestras actividades diarias y de mantener nuestro cuerpo fuerte y sano.'},
    {titulo:'¿DÓNDE LAS ENCUENTRO?',	texto:'Estas macromoléculas se encuentran en toda la materia viva. De hecho, podemos nutrirnos con ellas a través del consumo de productos animales como carnes, huevos y lácteos. Los granos, frutos secos y cereales también contienen proteínas, aunque su valor nutricional es menor.'},
    {titulo:'¿SABÍAS ESTO?',	texto:'Las proteínas están formadas por una cadena de aminoácidos que ayudan al cuerpo a regenerarse y a tener la capacidad de defenderse ante sustancias extrañas. Tambien permiten que tengamos huesos fuertes.'},
    {titulo:'¿SABÍAS ESTO?',	texto:'De acuerdo con la Biblioteca Nacional de Medicina de los Estados Unidos, la ingesta recomendada de proteínas para los adultos saludables, y en una diera de 2,000 calorías, es de 100 gramos. Esto representa el 20% de un total de calorías diarias.'},
];

export const matriz_video_lipidos =[

    {titulo:'¿CUÁNTAS LÍPIDOS NECESITAS CONSUMIR?',	texto:'Hay que tener en cuenta que el porcentaje de grasas que puede ingerir una persona, varía dependiendo de su metabolismo y su forma física. Y por supuesto, si hacemos ejercicio físico también puede cambiar, es decir, una persona sedentaria no puede ingerir el mismo porcentaje de grasas que una que corre a diario 10 kilómetros.', texto2:"Según la Organización Mundial de la Salud (OMS), reducir el consumo total de grasa a menos del 30% de la ingesta calórica diaria contribuye a prevenir el aumento de peso en la población adulta." , texto3:"Por lo que podemos deducir que la grasa total debe suponer entre un 30-35% de la energía total de la dieta. En lo que se refiere al colesterol, se recomienda no sobrepasar los 300 mg por persona y día."},
    {titulo:'¿QUÉ SON LOS LÍPIDOS?',	texto:'Los lípidos son un grupo heterogéneo de compuestos orgánicos. Dentro de ellos se encuentran las grasas, que se dividen en saturadas e insaturadas. Su estructura química varía y sus propiedades y funciones también dependiendo de los ácidos que contengan.'},
    {titulo:'¿POR QUÉ SON IMPORTANTES?',	texto:'Las grasas son necesarias para nuestro organismo, ya que proveen ácidos grasos esenciales que nuestro cuerpo no podría fabricar. Además, cumplen diversas funciones vitales como, por ejemplo, almacenar energía, sirven de estructura para producir hormonas y otras sustancias y facilitan el transporte de varias vitaminas.'},
    {titulo:'¿DÓNDE LAS ENCUENTRO?',	texto:'Encontramos este nutriente en alimentos como la manteca, tocino, mantequilla, nata, yema de huevo, carne magra, leche, aceite de coco, pescado y aceite de pescado, algas o alimentos como lácteos enriquecidos en Omega 3'},
    {titulo:'¿SABÍAS ESTO?',	texto:'Las grasas de la dieta son fundamentales para apreciar el gusto y aroma de los alimentos, y contribuyen a la sensación de saciedad tras la ingestión de los mismos.'},

];

/****BASE DE DATOS DE carbohidratos ****/

export const avena = [
    {
        nombre: 'Avena en hojuelas',
        calorias: '260kcal-118.04cal',
        libras: '1 lb',
        proteina: '16.89g',
        lipidos:'6.9g',
        carbohidratos: '66.27g',
        vitaminas: 'Vitamina B1',
        vitamina2: 'Vitamina B2',
        vitamina3: 'Vitamina B3',
        vitamina4: 'Vitamina B6',
        vitamina5: 'Vitamina B12',
        vitamina6: 'Vitamina E',

    }
]

export const harina = [
    {
        nombre: 'Harina de pancakes',
        calorias: '355kcal - 161.17cal',
        libras: '1 lb',
        proteina: '10g',
        lipidos:'1.76g',
        carbohidratos: '73.6g',
        vitamina1: 'Vitamina A',
        vitamina2: 'Vitamina B6',
        vitamina3: 'Vitamina B9',

    }
]
export const arroz = [
    {
        nombre: 'Arroz blanco',
        calorias: '360kcal - 163.44cal',
        libras: '1 lb',
        proteina: '6.61g',
        lipidos:'0.58g',
        carbohidratos: '79.34g',
        vitamina1: 'Ácido Fólico',
        vitamina2: 'Tiamina',
        vitamina3: 'Riboflavina',
        vitamina4: 'Niacina',
        vitamina5: 'Vitamina D',
        vitamina6: 'Vitamina E',

    }
]


/****BASE DE DATOS DE FRUTAS ****/

export const manzana = [
    {
        nombre: 'Manzana criolla con cáscara',
        calorias: '54kcal - 24.52cal',
        libras: '1 lb',
        proteina: '0.3g',
        lipidos:'0.1g',
        carbohidratos: '14.6g',
        vitaminas: 'Vitamina B1',
        vitamina2: 'Vitamina B2',
        vitamina3: 'Vitamina B6',
        vitamina4: 'Vitamina C',

    }
]

export const mango = [
    {
        nombre: 'Mango',
        calorias: '0.065kcal - 65cal',
        libras: '1 lb',
        proteina: '0.5g',
        lipidos:'0.3g',
        carbohidratos: '17.3g',
        vitamina1: 'Vitamina A',
        vitamina2: 'Vitamina B6',
        vitamina3: 'Vitamina C',
        vitamina4: 'Vitamina D',
        vitamina5: 'Vitamina E',

    }
]
export const papa = [
    {
        nombre: 'Papa con cáscara (cruda)',
        calorias: '77kcal - 34.96cal',
        libras: '1 lb',
        proteina: '2.02g',
        lipidos:'0.09g',
        carbohidratos: '17.47g',
        vitamina1: 'Ácido Fólico',
        vitamina2: 'Tiamina',
        vitamina3: 'Riboflavina',
        vitamina4: 'Niacina',
        vitamina5: 'Vitamina D',
        vitamina6: 'Vitamina E',

    }
]
/****BASE DE DATOS DE legumbres ****/

export const arverja = [
    {
        nombre: 'Arveja',
        calorias: '341kcal - 154.81cal',
        libras: '1 lb',
        proteina: '24.55g',
        lipidos:'1.16g',
        carbohidratos: '60.37g',
        vitamina1: 'Vitamina B1',
        vitamina2: 'Vitamina B2',
        vitamina3: 'Vitamina B6',
        vitamina4: 'Vitamina C',

    }
]

export const frijol = [
    {
        nombre: 'Frijol rojo',
        calorias: '337kcal - 153cal',
        libras: '1 lb',
        proteina: '22.53g',
        lipidos:'1.06g',
        carbohidratos: '61.26g',
        vitamina1: 'Vitamina A',
        vitamina2: 'Vitamina B1',
        vitamina3: 'Vitamina B2',
        vitamina4: 'Vitamina B9',
        vitamina5: 'Vitamina c',

    }
]
export const lentejas = [
    {
        nombre: 'lentejas',
        calorias: '77kcal - 34.96cal',
        libras: '1 lb',
        proteina: '2.02g',
        lipidos:'0.09g',
        carbohidratos: '17.47g',
        vitamina1: 'Ácido Fólico',
        vitamina2: 'Tiamina',
        vitamina3: 'Riboflavina',
        vitamina4: 'Niacina',
        vitamina5: 'Vitamina D',
        vitamina6: 'Vitamina E',

    }
]
/****BASE DE DATOS DE PROTEINAS ****/

export const Cerdo = [
    {
        nombre: 'Cerdo semimagra (asada)',
        calorias: '260kcal-118.04cal',
        libras: '1 lb',
        proteina: '27.63 g',
        lipidos:'15.76g',
        carbohidratos: '0g',
        vitaminas: 'Vitamina B1',
        vitamina2: 'Vitamina B2',
        vitamina3: 'Vitamina B3',
        vitamina4: 'Vitamina B6',
        vitamina5: 'Vitamina B12',
        vitamina6: 'Vitamina E',

    }
]

export const pollo = [
    {
        nombre: 'Pechuga de pollo (cruda)',
        calorias: '127kcal - 78-09cal',
        libras: '1.1 b',
        proteina: '20.85g',
        lipidos:'9.25g',
        carbohidratos: '0g',
        vitaminas: 'Vitamina B1',
        vitaminas: 'Vitamina B2',
        vitaminas: 'Vitamina B3',
        vitaminas: 'Vitamina B6',
        vitaminas: 'Ácido Fólico',
        vitaminas: 'Vitamina C',
    }
]
export const res = [
    {
        nombre: 'Res semimagra (cruda)',
        calorias: '264kcal - 119.86cal',
        libras: '1.1 b',
        proteina: '26.44g',
        lipidos:'11.39g',
        carbohidratos: '0g',
        vitamina: 'Vitamina A',
        vitamina1: 'Vitamina B6',
        vitamina2: 'Vitamina B12',
        vitamina3: 'Vitamina E',
        vitamina4: 'Ácido Pantoténico',
        vitamina5: 'Biotina',
    }
]




/****BASE DE DATOS DE LOS LACTEOS ****/

export const leche = [
    {
        nombre: 'Leche de vaca, integra (polvo)',
        calorias: '496kcal - 496cal',
        libras: '1L',
        proteina: '26.32g',
        lipidos:'26.71g',
        carbohidratos: '38.42g',
        vitamina1: 'Vitamina A',
        vitamina2: 'Complejo B,C y D',
        vitamina3: 'Vitamina B2',
        vitamina4: 'Vitamina B12',
        vitamina5: 'Biotina',
        vitamina6: 'Ácido Pantoténico',
    
    }
    
]

export const yogurt = [
    {
        nombre: 'Yogurt natural',
        calorias: '56kcal - 56cal',
        libras: '1 l b',
        proteina: '5.73g',
        lipidos:'0.18g',
        carbohidratos: '07.68g',
        vitamina1: 'Vitamina A',
        vitamina2: 'Vitamina D',
        vitamina3: 'Vitamina B2',
        vitamina4: 'Vitamina B3',
        vitamina5: 'Vitamina B12',
      
    }
]
export const Crema = [
    {
        nombre: 'Crema espesa (nata)',
        calorias: '345kcal - 345cal',
        libras: '1.L',
        proteina: '2.05g',
        lipidos:'37g',
        carbohidratos: '2.79g',
        vitamina1: 'Vitamina A',
        vitamina2: 'Vitamina B1',
        vitamina3: 'Vitamina B2',
        vitamina4: 'Vitamina B3',
        vitamina5: 'Vitamina B5',
        vitamina6: 'Vitamina B6',
       
    }
]
/*** BASE DE DATOS PESCADOS */

export const sardinas = [
    {
        nombre: 'Sardina',
        calorias: '186kcal - 84.44cal',
        libras: '1L',
        proteina: '20.86g',
        lipidos:'10.46g',
        carbohidratos: '0.74g',
        vitamina1: 'Vitamina B6',
        vitamina2: 'Vitamina B12',
        vitamina3: 'Vitamina E',
        vitamina4: 'Niacina',
        vitamina5: 'Vitamina D',
    }

]
export const Salmón = [
    {
        nombre: 'Salmón',
        calorias: '0.208kcal - 208cal',
        libras: '1 lb',
        proteina: '19,9g',
        lipidos:'13,6g',
        carbohidratos: '0g',
        vitamina1: 'Vitamina B6',
        vitamina2: 'Vitamina B12 ',
        vitamina3: 'Tiamina',
        vitamina4: 'Niacina',
        vitamina5: 'Vitamina D',
        vitamina6: 'Vitamina E',
      
    }
]
export const MojarraFresca = [
    {
        nombre: 'Mojarra fresca)',
        calorias: '106kcal - 48.12cal',
        libras: '1.L',
        proteina: '19.2g',
        lipidos:'2.7g',
        carbohidratos: '0g',
        vitamina1: 'Vitamina B',
        vitamina2: 'Vitamina B',
        vitamina3: 'Vitamina E',
        vitamina4: 'Fósforo',
        vitamina5: 'Calcio',
       
    }
]





/**DATAS PROTEINAS (SECCIÒN FRUTAS Y VEGETALES)**/
export const Aceitunas = [
    {
        nombre: 'Aceitunas',
        calorias: '0.126kcal-126cal',
        libras: '1 lb',
        proteina: '1,4g',
        lipidos:'12,7g',
        carbohidratos: '4,1g',
        vitaminas: 'Vitamina A',
        vitamina2: 'Vitamina E',



    }

]
export const Platano = [
    {
        nombre: 'Plátano',
        calorias: '0.085kcal-85cal',
        libras: '1 lb',
        proteina: '1,2g',
        lipidos:'0,3g',
        carbohidratos: '19,5g',
        vitaminas: 'Vitamina B',
        vitamina2: 'Vitamina C',
        vitamina3: 'Vitamina E',
        vitamina4: 'Vitamina K',


    }

]
export const Guayaba = [
    {
        nombre: 'Guayaba',
        calorias: '0.051kcal-51cal',
        libras: '1 lb',
        proteina: '0,8g',
        lipidos:'0,6g',
        carbohidratos: '11,9g',
        vitaminas: 'Vitamina A',
        vitamina2: 'Vitamina C',
        vitamina3: 'Vitamina E',
        vitamina4: 'Vitamina D12',


    }

]

/***DATAS LIPIDOS ****/

/*-------ACEITES Y GRASAS------------*/
export const AceiteOliva = [
    {
        nombre: 'Aceite de oliva',
        calorias: '884kcal-884cal',
        libras: '1L',
        proteina: '0g',
        lipidos:'100g',
        carbohidratos: '0g',
        vitaminas: 'Vitamina A',
        vitamina2: 'Vitamina D',
        vitamina3: 'Vitamina E',
        vitamina4: 'Vitamina K',
    }
]

export const Margarina = [
    {
        nombre: 'Margarina',
        calorias: '175kcal - 175cal',
        libras: '1L',
        proteina: '0g',
        lipidos:'19.56g',
        carbohidratos: '0.4g',
        vitamina1: 'Vitamina A',
        vitamina2: 'Vitamina B3',
        vitamina3: 'Vitamina B5',
        vitamina4: 'Vitamina D',
        vitamina5: 'Vitamina E',


    }
]

export const Mayonesa = [
    {
        nombre: 'Mayonesa',
        calorias: '390kcal - 390cal',
        libras: '1L',
        proteina: '0.9g',
        lipidos:'33.4g',
        carbohidratos: '23.9g',
        vitamina1: 'Vitamina A',
        vitamina2: 'Vitamina B1',
        vitamina3: 'Vitamina B5',
        vitamina4: 'Vitamina B12',
        vitamina5: 'Vitamina E',
        vitamina6: 'Vitamina K',


    }
]

/***FRUTA Y VEGETALES */


export const Aceitunas2 = [
    {
        nombre: 'Aceituna',
        calorias: '0.126kcal-124cal',
        libras: '1lb',
        proteina: '1.4g',
        lipidos:'12.7g',
        carbohidratos: '4.1g',
        vitamina1: 'Vitamina A',
        vitamina2: 'Vitamina E',
    }
]

export const coco = [
    {
        nombre: 'coco',
        calorias: '1481kcal - 354cal',
        libras: '1Lb',
        proteina: '3.33g',
        lipidos:'33.496g',
        carbohidratos: '15.23g',
        vitamina1: 'Vitamina A',
        vitamina2: 'Vitamina B3',
        vitamina3: 'Vitamina B5',
        vitamina4: 'Vitamina D',
        vitamina5: 'Vitamina E',


    }
]

export const Aguacate = [
    {
        nombre: 'Aguacate',
        calorias: '160kcal - 72.64cal',
        libras: '1Lb',
        proteina: '2g',
        lipidos:'14.66g',
        carbohidratos: '8.53g',
        vitamina1: 'Vitamina B3',
        vitamina2: 'Vitamina B5',
        vitamina3: 'Vitamina B6',
        vitamina4: 'Vitamina c',
        vitamina5: 'Vitamina E',
        vitamina6: 'Vitamina K',


    }
]
